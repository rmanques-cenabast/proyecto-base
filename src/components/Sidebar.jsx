...
function SidebarItem({ icon, label, to }) {
  return (
    <Link to={to} className="flex flex-row items-center px-3 py-2 gap-1.5 w-sidebar-item h-sidebar-item-h rounded-button cursor-pointer no-underline">
...
function Dropdown({ icon, label, children }) {
  ...
  return (
    <div>
      <div className="flex flex-row items-center px-3 py-2 gap-1.5 w-sidebar-item h-sidebar-item-h rounded-button cursor-pointer" onClick={() => setOpen(v => !v)} aria-expanded={open}>
        <div className="w-4 h-4">{icon}</div>
        <div className="flex-1 min-w-0 max-w-[200px] h-6 font-roboto text-white text-base leading-6 overflow-hidden text-ellipsis whitespace-nowrap">{label}</div>
        <div className="flex items-center ml-1">
          <img src="/iconSideButton.svg" alt={open ? "collapse" : "expand"} width="16" height="16" className={`!w-4 !h-4 !max-w-4 !max-h-4 block transition-transform duration-[180ms] ease-in-out ${open ? 'rotate-0' : 'rotate-180'}`} />
        </div>
      </div>
            ...
            if (it.type === 'dropdown') return (
              <Dropdown key={idx} icon={<img src={it.icon} alt={it.label} width="16" height="16" />} label={it.label}>
                {it.children.map((c, ci) => {
                  ...
                  return <Link key={ci} to={`/gestion/${slug}`} className="flex flex-row items-start px-3 py-2 gap-2 w-dropdown-item h-9 rounded-button text-white font-roboto text-sm leading-5 no-underline">{c}</Link>
                })}
              </Dropdown>
            )
            return null
